    ...
    static func analyzeJson(view: TimeLineView?, model: TimeLineViewModel?, json: [String: Any], acct: inout String, isMerge: Bool = false, isPinned: Bool? = nil) -> ContentData {
        ...
        if let media_attachments = media_attachments {
            for media_attachment in media_attachments {
                ...
                let description = media_attachment["description"] as? String
                var width = 0
                var height = 0
        ...
        let content = reblog?["content"] as? String ?? json["content"] as? String
        ...
        if reblog_acct == nil {
            if let pollJson = json["poll"] as? [String: Any] {
                poll = getPoll(json: pollJson)
            } else {
                poll = nil
            }
        } else {
            if let pollJson = reblog?["poll"] as? [String: Any] {
                poll = getPoll(json: pollJson)
            } else {
                poll = nil
            }
